...
void CApp::gerer()
{
	...
	if (COPY == NULL && !IO->pressed(CInput::MLeft) && !IO->pressed(CInput::MRight))
	{
		for (int i=0; i < CALQUE_LIST.size(); i++)
			if (CALQUE_LIST[i]->changed)
			{
				...
				RETOUR_VECTOR.push_back(CCalque::_t); CRetour::vector.push_back(IO);
				RETOUR->add_retour();

				CCalque::_t->creer_retour(&CCalque::_t->getTexture()->copyToImage(), CCalque::ACT);
				IO->creer_retour();
			}
	}
	...
	{
		//Utiliser l'outil
		if (OUTIL != NULL || COPY != NULL && COutil::isTransform())
			OUTIL->use();
		//Copie
