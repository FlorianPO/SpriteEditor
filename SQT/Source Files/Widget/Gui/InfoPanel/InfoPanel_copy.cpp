...
InfoPanel_copy::InfoPanel_copy(QWidget *parent, const QPoint& position) : QWidget(parent) {
	move(position);
...
void InfoPanel_copy::bind(Copy* copy) {
	...
	lambdas.push_back(QObject::connect(SizeX, &ClickLabel::valueChanged, [copy](int x) {copy->setGlobalPosition(sf::Vector2f(x, copy->getGlobalPosition().y));}));
	// SIZEY
	lambdas.push_back(QObject::connect(copy, &Copy::copyMoved, [this](sf::Vector2f pos) {SizeY->changeValue(pos.y);}));
	lambdas.push_back(QObject::connect(SizeY, &ClickLabel::valueChanged, [copy](int y) {copy->setGlobalPosition(sf::Vector2f(copy->getGlobalPosition().x, y));}));
	// SIZEW
