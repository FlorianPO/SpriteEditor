...
void SelecColor::pickWithSelec() {
	...
	FOR_J (test_image->getSize().y) {
		FOR_I (test_image->getSize().x) {
			if (SELEC->onSelec(sf::Vector2i(i + LAYER->getPosition().x, j + LAYER->getPosition().y)) && pixelCheck(test_image->getPixel(i, j))) {
				if (j == 0 || !SELEC->onSelec(sf::Vector2i(i + LAYER->getPosition().x, j - 1 + LAYER->getPosition().y)) || !pixelCheck(test_image->getPixel(i, j - 1))) {
					line[0].position = sf::Vector2f(i, j);
					line[1].position = sf::Vector2f(i + 1, j);
					oriented_lines.push_back(oLine(line, UP, i, j));
				}
				if (i == test_image->getSize().x - 1 || !SELEC->onSelec(sf::Vector2i(i + 1 + LAYER->getPosition().x, j + LAYER->getPosition().y)) || !pixelCheck(test_image->getPixel(i + 1, j))) {
					line[0].position = sf::Vector2f(i + 1, j);
					line[1].position = sf::Vector2f(i + 1, j + 1);
					oriented_lines.push_back(oLine(line, RIGHT, i, j));
				}

				if (j == test_image->getSize().y - 1 || !SELEC->onSelec(sf::Vector2i(i + LAYER->getPosition().x, j + 1 + LAYER->getPosition().y)) || !pixelCheck(test_image->getPixel(i, j + 1))) {
					line[0].position = sf::Vector2f(i + 1, j + 1);
					line[1].position = sf::Vector2f(i, j + 1);
					oriented_lines.push_back(oLine(line, DOWN, i, j));
				}
				if (i == 0 || !SELEC->onSelec(sf::Vector2i(i - 1, j)) || !pixelCheck(test_image->getPixel(i - 1 + VECTOR2I(LAYER->getPosition()).x, j + VECTOR2I(LAYER->getPosition()).y))) {
					line[0].position = sf::Vector2f(i, j + 1);
					line[1].position = sf::Vector2f(i, j);
					oriented_lines.push_back(oLine(line, LEFT, i, j));
				}
