...
void Pot::full_dessiner(sf::Color color) {
...
void Pot::dessiner() {
	...
	while (!ranges.empty()) {
		...
		for (int i = range.debutX; i <= range.finX; i++) {
			if (range.Y > 0 && bit_image.getPixel(i, range.Y - 1).r == 0 && SELEC->onSelec(i + VECTOR2I(LAYER->getPosition()).x, range.Y - 1 + VECTOR2I(LAYER->getPosition()).y) && pixelCheck(test_image->getPixel(i, range.Y - 1)))
				linearFill(i, range.Y - 1);

			if (range.Y < image.getSize().y - 1 && bit_image.getPixel(i, range.Y + 1).r == 0 && SELEC->onSelec(i + VECTOR2I(LAYER->getPosition()).x, range.Y + 1 + VECTOR2I(LAYER->getPosition()).y) && pixelCheck(test_image->getPixel(i, range.Y + 1)))
				linearFill(i, range.Y + 1);
		}
	}
	sf::Texture texture;
...
void Pot::linearFill(int x, int y) {
	...
	} while (i_left >= 0 && bit_image.getPixel(i_left, y).r == 0 && SELEC->onSelec(i_left + VECTOR2I(LAYER->getPosition()).x, y + VECTOR2I(LAYER->getPosition()).y) && pixelCheck(test_image->getPixel(i_left, y)));
	i_left++;
	...
	} while (i_right <= bit_image.getSize().x - 1 && bit_image.getPixel(i_right, y).r == 0 && SELEC->onSelec(i_right + VECTOR2I(LAYER->getPosition()).x, y + VECTOR2I(LAYER->getPosition()).y) && pixelCheck(test_image->getPixel(i_right, y)));
	i_right--;
